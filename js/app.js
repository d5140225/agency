(() => {
    ...
    function windowLoad() {
        ...
        let sections = document.querySelectorAll(".about__counter");
    ...
    if (animItems.length > 0) {
        ...
        function animOnScroll() {
            for (let index = 0; index < animItems.length; index++) {
                ...
                if (pageYOffset > animItemOffset - animItemPoint && pageYOffset < animItemOffset + animItemHeight) animItem.classList.add("_animated"); else if (!animItem.classList.contains("_anim-no-hide")) animItem.classList.remove("_animated");
            }
        }
        function offset(el) {
    ...
    let script_button = document.getElementById("playButton");
    let displayStatus = document.getElementById("displayStatus");
    video.onloadedmetadata = function() {
        let minutesDuration = Math.floor(video.duration / 60);
        let secondsDuration = Math.floor(video.duration - 60 * minutesDuration);
        let minuteValueDuration = minutesDuration.toString().padStart(1, "0");
        let secondValueDuration = secondsDuration.toString().padStart(2, "0");
        let durationTime = `${minuteValueDuration}:${secondValueDuration}`;
        displayStatus.textContent = `${durationTime}`;
    };
    function playPause() {
        if (video.paused) {
            video.play();
            script_button.classList.add("_paused");
        } else {
            video.pause();
            script_button.classList.remove("_paused");
        }
    }
    script_button.addEventListener("click", playPause);
    function setTime() {
        let minutesDuration = Math.floor(video.duration / 60);
        let secondsDuration = Math.floor(video.duration - 60 * minutesDuration);
        let minuteValueDuration = minutesDuration.toString().padStart(1, "0");
        ...
        let seconds = Math.floor(video.currentTime - 60 * minutes);
        let minuteValue = minutes.toString().padStart(1, "0");
        let secondValue = seconds.toString().padStart(2, "0");
        let mediaTime = `${minuteValue}:${secondValue}`;
        displayStatus.textContent = `${durationTime} / ${mediaTime}`;
    }
